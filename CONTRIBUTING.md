...
```
You may type this line on your own when writing your commit messages. However, if your user.name and user.email are set in your git configs, you can use `git commit -s` (or `--signoff`) to add the `Signed-off-by` line to the end of the commit message. The `dco` workflow checks every commit of a pull request for the trailer and fails when one lacks it.
