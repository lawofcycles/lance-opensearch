...
- AND / OR operator control and phrase order (with slop) for Lance FTS via the `lance_match` and `lance_match_phrase` DSL queries.
...
- OpenSearch's stock `match` and `match_phrase` queries against a `lance_text` field ignore the `operator`, `minimum_should_match`, phrase order, and `slop` parameters. Reason: `lance_text` uses a keyword-analyzer `TextSearchInfo` so the whole query string reaches Lance as a single token and Lance's own tokenizer runs on the query text — OpenSearch's combining layer never sees multiple tokens. Use the plugin's `lance_match` DSL for AND / OR operator control and `lance_match_phrase` for phrase order and slop; both push the raw text and the requested semantics into `FullTextQuery.match` / `FullTextQuery.phrase` on Lance directly. `fuzziness`, `minimum_should_match`, and `prefix_length` are rejected with 400 because Lance's Java SDK does not expose them yet.
