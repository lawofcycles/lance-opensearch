...
public class TransportLanceCoordinatorActionTests extends OpenSearchTestCase {
    ...
    public void testResponseHandlerRunsOnTheTransportThread() {
        FragmentFanOut fanOut = new FragmentFanOut(2, coordinatorPool(), responses -> {}, new CountingListener());

        assertEquals(ThreadPool.Names.SAME, fanOut.handler(0).executor());
        assertEquals(ThreadPool.Names.SAME, fanOut.handler(1).executor());
    }

    public void testMergeRunsOnCoordinatorPoolInSlotOrderAfterLastResponse() throws Exception {
        CountingListener done = new CountingListener();
    ...
    public void testRejectedMergeSubmitFailsListenerOnceWithRejection() throws Exception {
        ...
        try {
            // Occupy the single thread and the single queue slot.
    ...
    /** Counts completions and lets a test wait for the first one. */
    private static final class CountingListener implements ActionListener<Void> {
        ...
        void await() throws InterruptedException {
            ...
            // A second completion, if any, would arrive right behind
            // the first; give it a moment so the counts below catch it.
            Thread.sleep(50);
        }
    }
}
