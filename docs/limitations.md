...
- Lance-backed indices are always single-shard. `POST /_lance/attach` rejects any request carrying `number_of_shards` with 400. Fragment path fans out per fragment regardless of shard count, so shards no longer influence search parallelism. Multi-node parallelism comes from replica placement (`index.number_of_replicas: data_nodes - 1`, or `auto_expand_replicas: 0-all`); attach leaves `number_of_replicas` at 0, so a fresh index runs on one node until the operator changes it.
- Multi-node `_search` fan-out covers the nodes that hold a started shard copy at the time of the request. A node whose copy is still initialising takes no fragments, so the spread is only as wide as the recovered copies. Fragments are assigned round-robin by fragment id without regard to size, and a request fails as a whole when any one node fails.
...
- Full-text queries (`lance_match` and the other Lance FTS shapes) scan Lance without a fragment list when the executing node holds every fragment of the table, which is always the case today because the fan-out is pinned to the primary's node; the inverted-index lookup then does not depend on the row count. If a node ever executes a proper subset of the fragments (a future split of the fan-out across data nodes), its scan carries that subset and Lance 12 reads the `_rowid` column of those fragments as a prefilter before the index lookup, so latency grows with the row count of the subset. Removing that prefilter needs a Lance-side change (a fragment-bitmap prefilter instead of a row id read).
