...
- Bucket trees deeper than three levels, two bucket aggregations under the same parent, a bucket under a `composite`, or a nested `terms` tree whose estimated key combinations (the product of the levels' `shard_size`) exceed `lance.aggregation.pushdown_max_groups`.
