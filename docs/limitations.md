...
- Full-text queries (`lance_match` and the other Lance FTS shapes) scan Lance without a fragment list when the executing node holds every fragment of the table, which is the case with the default single shard copy; the inverted-index lookup then does not depend on the row count. When shard copies are on several nodes, each node executes a proper subset of the fragments, its scan carries that subset, and Lance 12 reads the `_rowid` column of those fragments as a prefilter before the index lookup, so FTS latency grows with the row count of the subset. Removing that prefilter needs a Lance-side change (a fragment-bitmap prefilter instead of a row id read).
