...
- Lance-backed indices are always single-shard. `POST /_lance/attach` rejects any request carrying `number_of_shards` with 400. Fragment path fans out per fragment regardless of shard count, so shards no longer influence search parallelism. Multi-node parallelism comes from the shard copies that `index.auto_expand_replicas: 0-all` (set by attach and by the namespace surface) places on every data node.
- A data node without a started shard copy cannot execute fragments yet: the executor needs the node's `IndexService` for the query context and the security reader wrapper, so the coordinator skips nodes whose copy is missing or still initialising and the spread is only as wide as the recovered copies. Running fragments on copy-less nodes is tracked in [#73](https://github.com/lawofcycles/lance-opensearch/issues/73). Fragments are assigned round-robin by fragment id without regard to size, and a request fails as a whole when any one node fails.
...
- Full-text queries (`lance_match` and the other Lance FTS shapes) scan Lance without a fragment list when the executing node holds every fragment of the table, which is the case on a single data node; the inverted-index lookup then does not depend on the row count. With several data nodes each node executes a proper subset of the fragments, its scan carries that subset, and Lance 12 reads the `_rowid` column of those fragments as a prefilter before the index lookup, so FTS latency grows with the row count of the subset. Removing that prefilter needs a Lance-side change (a fragment-bitmap prefilter instead of a row id read).
...
- If the table is not reachable, the engine fails to open (`Dataset at path ... was not found`) and every allocation attempt up to `index.allocation.max_retries` reports that error in `_cluster/allocation/explain`. The index stays red with the primary `UNASSIGNED` / `ALLOCATION_FAILED`. `_count` returns 503 (`all shards failed`) and `_search` returns 400 with the Lance not-found message because the fragment path cannot open the table either. Once the retries are exhausted the restore itself is marked failed, so `POST /_cluster/reroute?retry_failed=true` does not help even after the table is back: OpenSearch refuses to allocate a primary whose restore has failed. Delete the index and restore it again.
