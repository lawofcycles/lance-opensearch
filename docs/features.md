...
- `track_total_hits` follows the OpenSearch contract: omitted counts up to 10,000 and reports `{"value": 10000, "relation": "gte"}` beyond that, an integer sets that bound, `true` counts exactly, `false` omits `hits.total`. The coordinator ships the bound to every executor and composes the relation from the per-node counts; whenever the relation is `gte` the value is the bound itself, as on an ordinary index, also when the per-node counts of several nodes add up to less than the bound because each node counted its own fragments' share of a scan stopped at the bound.
  ...
  - A Lance FTS query (`match`, `lance_*`, or a `bool` collapsed into a prefiltered FTS scan) is counted from the scan that served the hits or aggregations. Under a bound the page's scan asks Lance for `max(size, bound + 1)` rows instead of `size`: the page is the top `size` of them by score, and the number of rows the scan returned is the count, exact when it came back short of the limit and a lower bound (reported as `gte` at the bound) when it filled, so one scan of the inverted index serves the page and `hits.total`. Lance rebuilds the index's document set on every FTS scan, so a second, count only scan would cost as much as the page. With `track_total_hits: true` the page scan keeps `size` rows and a count only scan of the whole match set follows; with `track_total_hits: false` the page scan keeps `size` rows and nothing is counted. A `size: 0` request without aggregations runs the count only scan alone, stopped at `bound + 1` rows under a bound.
