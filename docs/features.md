...
- `native_memory`: `estimated_bytes` (what the `lance_native` breaker currently accounts for, as of the last sampler tick), `session_bytes` (`Session.sizeBytes()` of the shared Lance session, read live), `column_store_bytes` (the same value as `column_store.bytes`). The breaker estimate is the sum of the other two as sampled up to one `lance.native_memory.circuit_breaker.poll_interval` ago.
