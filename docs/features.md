...
- `_search` (fragment path) and `GET /_doc/{id}` (engine path) do not share a freshness view. Fragment path reads the latest version per query (through the snapshot cache below); engine path advances only with the poll cadence. See [limitations.md](limitations.md).

## Fragment path snapshot and column cache
...
- Snapshots are closed when the namespace poll sees the table (or its tag) move to another version, when the index is deleted, when `lance.cache.enabled` is set to `false`, or when they are the least recently used ones above `lance.cache.max_snapshots`; a snapshot a request still holds closes when that request ends. Closing a snapshot releases its columns.
- Settings: `lance.cache.enabled` (dynamic, default `true`; `false` retires every snapshot, and from then on each request opens the table, runs the schema pass and loads its columns into heap for itself; concurrent requests against the same index do not share any of that, the same as before the cache existed), `lance.cache.max_snapshots` (static, default 64), `lance.cache.column_share` (static, default 0.4: the fraction of `lance.native_memory.limit` the column store may use; see "Native memory bounds").
- Not cached yet: keyword columns (heap, per request) and the shard path (`GET /_doc/{id}`, `_count`, stats), which keeps its own reader per manifest version.
...
- **`lance.native_memory.limit`** — Absolute (`10gb`) or a percentage of host memory left after the JVM heap (`40%`, default). `lance.cache.column_share` (default 0.4) of it goes to the off-heap column store; the rest is split 6:1 between the index cache and metadata cache. Static, node scope; rolling restart to change. The resolved split is logged at startup.
