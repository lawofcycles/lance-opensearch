...
Reference of what the plugin surfaces on a Lance-backed OpenSearch index. Grouped by concern so each subsection can be read on its own; start with [getting-started.md](getting-started.md) for a walkthrough that stitches these together. Three concerns have their own page: [admission.md](admission.md), [mapping-overrides.md](mapping-overrides.md) and [aggregations.md](aggregations.md).
...
Distribution over the cluster is automatic: fragments are spread over every data node that runs the plugin, and no replica setting is involved. A node does not need a shard copy of the index to execute its share.

- The executor builds its query context (mapping, `QueryShardContext`, the security plugin's reader wrapper) from the index metadata in cluster state. On the one node that hosts the shard it reuses the node's own `IndexService`; on every other node it creates a temporary `IndexService` for the duration of the request through the same `onIndexModule` hooks a regular index goes through. Lance fragments are read from external storage by whichever node gets them.
- Fragments are dealt round-robin by fragment id to the data nodes sorted by node id. Each node returns its top `from + size` hits, and the coordinator merges them by sort value (or score) with ties broken by node id order, then applies `from` / `size`. `search_after` is applied by each node before the merge.
- A failure on any one node fails the request; there is no partial-result mode.
- A full-text lookup is run by every node against the whole table's inverted index (no fragment list is passed to Lance) and each node keeps the rows of its own fragments, so the per-node results are disjoint shares of the same global result and the lookup cost does not depend on the row count. See limitations.md for the probe limit that bounds this on shapes needing every match.
...
- `POST /_lance/build_indexes/{index}` triggers Lance-side FTS / scalar / vector index builds from OpenSearch. Automatic builds happen for tables at or under `lance.builder.max_rows` (default 1,000,000 rows); larger tables use this explicit endpoint.
  ...
  - The response reports every target column under one of three keys, each split by index kind (`fts`, `scalar`, `vector`).
    - `built`: `{"fts": [{"column": "...", "type": "INVERTED"}], "scalar": [...], "vector": [...]}`, the column and the index type that was committed; with `optimize: true` the Lance index name and the type Lance reports.
    - `skipped`: `{"fts": [{"column": "...", "reason": "..."}], ...}`. Reasons: an index of the requested type already exists on the column (the reason names the type, for example `BTREE index already exists on [id]; use optimize=true to extend it over new fragments`), the preference is `none`, the table has fewer rows than the chosen vector type's training minimum, or with `optimize: true` the column has no index to extend.
    - `failed`: same shape, `reason` is the message Lance threw, for example `LanceError(IO): Permission denied (os error 13)` when the OpenSearch process cannot write into the table.
  - Status: 200 when `failed` is empty (skips are not failures), 400 when every failure is Lance rejecting the input (unknown `tokenizer`, malformed index parameters), 500 for anything else Lance threw. The body carries `built`, `skipped` and `failed` in all three cases, so a build that landed on some columns and failed on others shows both.
  - A build that stops before Lance runs (unknown index 404, a `columns` entry that is not indexable 400, `fts_columns` naming a non-Utf8 column 400) returns the usual error body instead.
- `index.lance.index_placement` (index setting, written by attach from the body key `"index_placement"`, values `in_table` / `node_local`, default `in_table`, final) chooses where those builds commit. The default commits into the source table's own manifest chain. Use `node_local` for tables on read-only mounts or object-store prefixes the OpenSearch process cannot write to.
...
- Freshness runs on the node holding the shard. Every started Lance-backed shard registers with that node's freshness service (a Lance-backed index has one shard and no replica, so exactly one node checks each index) and is checked at the cadence.
  - The check opens the table's latest manifest, resolves the followed tag if any, and compares the target version with the version the shard's reader serves. When they differ it re-derives the mapping, applies it only when it differs from the current one, refreshes the shard, which is where the engine swaps its reader, and retires the previous version's snapshot from the cache.
  - The mapping is re-derived from the version about to be served, with the stored `index.lance.overrides` re-applied, and rewritten first when a column they name was renamed or reset.
  - The derived mapping is merged into the shard's own mapper service as a preflight and the result compared with the current mapping, so an append without a schema change sends no mapping update and creates no cluster state task.
...
- Any other query is built by the executor from the request's own builder through the field types (the Lucene composition, `unplanned` naming the query type) and evaluated over the fragment leaves. The leaves serve doc values for every scalar column and answer a `term`, `range`, `exists`, `prefix`, `wildcard`, `regexp` and `fuzzy` on a `keyword` or numeric field the way an index with `index: false, doc_values: true` does, and a full text clause on a `lance_text` field through Lance.
- The predicate carries no score (every row it selects scores `1.0`), so the queries whose only effect is on scores translate only where no score is read: a count, an aggregation request, or a page ordered by stored columns. A page in score order (no `sort`, or a sort naming `_score`), `track_scores`, `min_score`, `rescore` and `collapse` read scores, and those queries then take the Lucene composition (`query type [dis_max] on a scored request`), which orders the page the way the query asks.

| Query | Evaluated by |
|---|---|
| `match_all`, `match_none` | Lance scan |
| `term`, `terms`, `range`, `exists`, `ids` | Lance scan |
| `prefix`, `wildcard`, `regexp` | Lance scan |
| `bool` | Lance scan |
| `constant_score`, `dis_max`, `boosting`, `function_score` | Lance scan where no score is read, Lucene composition otherwise |
| `match`, `match_phrase`, `multi_match` (`best_fields`) on `lance_text` | Lance inverted index |
| `lance_match`, `lance_match_phrase`, `lance_multi_match`, `lance_fts_bool`, `lance_fts_boost`, `lance_knn` | Lance inverted or vector index |
| `match` on a `keyword`, `multi_match` of another type, `fuzzy`, `terms_set`, `simple_query_string`, `query_string`, `script`, `script_score`, `wrapper` | Lucene composition |
| `nested` | Lucene composition |
| `geo_distance`, `geo_bounding_box`, `geo_polygon` on a `geo_point` override | Lucene composition |
| `intervals`, `match_phrase_prefix` | Refused |
| `span_*`, `more_like_this` | Refused |
| `has_child`, `has_parent`, `parent_id` | Refused |

Notes per row:

- `match_none` is the predicate `false`.
...
- `prefix`, `wildcard`, `regexp`: over the raw stored string of a `keyword` or `lance_text` column, see [Full-text search](#full-text-search); Lucene only regexp operators are refused.
...
- `constant_score`, `dis_max`, `boosting`, `function_score`: the predicate is the filter, the union of the clauses, the positive clause, and the query. A `function_score` with `min_score` or a `script_score` function, and the `script_score` query, take the Lucene composition under every envelope (the script runs there, and fails there).
- `match`, `match_phrase`, `multi_match` on `lance_text`: rewritten to the `lance_*` clause with the same parameters; a `bool` of such clauses fuses into one `lance_fts_bool` with scalar `filter` / `must_not` clauses as the scan's prefilter. See [Full-text search](#full-text-search).
- `lance_*` queries: the explicit forms; `lance_knn` with its inner `filter` as the prefilter.
- Lucene composition row: `wrapper` unwraps at the coordinator rewrite and its inner query takes that query's row. A `fuzzy` on `lance_text` is a Lance fuzzy match, on a `keyword` a doc values automaton. `simple_query_string` and `query_string` build their terms, phrases, prefixes and fuzzy terms through the field types, so on `lance_text` each becomes a Lance query and on a `keyword` a doc values query; `default_field: *` reaches every field, a vector column included, so name the fields.
- `nested`: the parent join over the hidden child documents, see [Nested](#nested-liststruct).
- `geo_*` queries: over the encoded doc values and a flat point tree, see [Mapping overrides](#mapping-overrides).
- `intervals`, `match_phrase_prefix`: the field type answers 400 on both paths (`Can only use interval queries on text fields - not on [body] which is of type [lance_text]`, `Can only use phrase prefix queries on text fields - not on [body] which is of type [lance_text]`).
- `span_*`, `more_like_this`: both need positions or term statistics Lance's inverted index does not surface through the Java SDK. A span query extracts a Lucene term from the field type's term query, which the Lance full text query does not carry (400, `Cannot extract a term from a query of type class org.opensearch.lance.query.LanceFtsQuery: ...`); `more_like_this` accepts `text` and `keyword` fields only (400, `more_like_this only supports text/keyword fields: [body]`). See [limitations.md](limitations.md#query-types-the-field-types-refuse).
- `has_child`, `has_parent`, `parent_id`: no join field is derived from a Lance table, so the join module answers 400 on both paths (`[has_child] no join field has been configured`). The stock `knn` query is the k-NN plugin's, see [Vector nearest neighbour](#vector-nearest-neighbour).
...
- Stock OpenSearch syntax on a `lance_text` field is BM25 scored by Lance's inverted index: `match` (with `operator`, `fuzziness`, `prefix_length`, `max_expansions`, `boost`, `_name`), `match_phrase` (with `slop`), `multi_match` of type `best_fields` (the default, with per field boosts such as `title^2` and a shared `operator`), and `bool` around them.
  ...
  - A clause with an explicit `analyzer` keeps OpenSearch side analysis (one Lance match per token, combined by Lucene).
  - `multi_match` of type `most_fields`, `cross_fields`, `phrase`, `phrase_prefix` or `bool_prefix`, with `fuzziness`, with a `tie_breaker`, or naming a field pattern or a field outside `lance_text` keeps its stock form (one Lance match per field, combined by Lucene).
  - The rewrite descends through `bool` and `dis_max`. Inside any other compound (`function_score`, `nested`, `constant_score`, `boosting`) the field type answers the stock clause itself: `match` as a Lance match over the whole text (the `operator` is then not applied), `match_phrase` of two or more words as a Lance phrase with its `slop`, `match` with `fuzziness` and `fuzzy` as a Lance fuzzy match.
...
- OpenSearch analyzer mode: a `type: text_analyzer` override on the attach body makes all of the above run over a derived tokens column the plugin backfilled with the declared built-in analyzer, with the query text analyzed the same way first, so the stemming, stop words and language handling of the OpenSearch analyzer apply to a Lance-backed column. [mapping-overrides.md](mapping-overrides.md#type-text_analyzer) describes the override, the backfill and the mapping flip.
- A `bool` holding full text clauses runs as one Lance FTS scan when its `must` and `should` lists hold full text clauses only (stock or `lance_*`), its `filter` holds scalar clauses only, and its `must_not` holds any mix. The scalar clauses become the scan's SQL prefilter (`filter(sql)` + `prefilter(true)`), the same way `lance_knn`'s `filter` does, and several full text clauses fuse into one `lance_fts_bool` under their occurrences.
  - Scalar clauses are `term`, `terms`, `range`, `exists`, `match_all`, `wildcard`, `regexp`, `prefix`, or a `bool` of those on mapped columns.
  - Lance scores the fused bool as Lucene's `BooleanQuery` would: required clauses intersect and add their scores, optional clauses add theirs when they match, prohibited clauses exclude.
  - Lance evaluates the predicate first, through the column's scalar index when it has one, and looks up the inverted index only for the selected rows; `hits.total.value` comes from the same prefiltered scan.
  - The bool must have no `minimum_should_match` and a boost of `1.0`; a clause's own boost is kept, on a `match` / `lance_match` / `multi_match` / `lance_multi_match` clause when several clauses fuse.
  - Kept on the Lucene side with unchanged results, every full text clause then running as its own Lance scan and Lucene combining them: a `should` next to a `filter` without a `must` (the full text clause is optional in Lucene, required in Lance's boolean), a scalar clause in `must` or `should`, a full text clause below `filter`, a clause the translator cannot express (multi-field sub-fields, unmapped fields), or a security plugin reader wrapper (DLS / FLS).
- `wildcard`, `regexp` and `prefix` on a `lance_text` field run as a Lance scan filter over the raw stored string of the column, not over the analyzed tokens (Lance's inverted index has no wildcard or regexp query type). A pattern has to match the whole stored value the way it matches a whole keyword term, the analyzer is not applied to the pattern or the value, and matching is case sensitive unless the query sets `case_insensitive: true`. The same three queries on a `keyword` column take the same SQL.
  - `wildcard` becomes `LIKE` / `ILIKE` (`*` to `%`, `?` to `_`, literal `%` / `_` / `\` escaped).
  - `prefix` becomes `starts_with` (`lower()` on both sides when case insensitive).
...
- `with_position` (boolean, default `false`, the same as Lance's default) makes Lance store token positions in the inverted indexes this request creates. `lance_match_phrase` needs them; `lance_match`, `lance_multi_match`, `lance_fts_bool` and `lance_fts_boost` do not, and an index with positions is larger. Like `tokenizer`, it requires `fts_columns` and is not accepted with `optimize`.
  - Whether positions are stored is fixed when the index is created, so a column built without them has to be dropped on the Lance side and built again to gain phrase support.
...
- Japanese and Chinese text has no word separators, so `simple` indexes a whole sentence as one token and word queries miss. `icu` (ICU dictionary segmentation, compiled into the Lance native library) works with no setup. `lindera/ipadic`, `lindera/unidic`, `jieba/default` need a dictionary under `$LANCE_LANGUAGE_MODEL_HOME` (default `~/.local/share/lance/language_models` on Linux) on every data node, readable by the OpenSearch process.
  - Lindera: download `lindera-ipadic-<version>.zip` from the [lindera releases](https://github.com/lindera/lindera/releases), unpack it to `$LANCE_LANGUAGE_MODEL_HOME/lindera/ipadic/main`, and write `$LANCE_LANGUAGE_MODEL_HOME/lindera/ipadic/config.yml` containing `segmenter: {mode: normal, dictionary: <absolute path to main>}`.
  - Jieba: place jieba-rs's `dict.txt` at `$LANCE_LANGUAGE_MODEL_HOME/jieba/default/dict.txt`.
  - Lance loads the dictionary when it builds the index and again whenever it opens the index for a query, so the files must stay in place on every node that serves the index.
...
- `_source` and `_id` are synthesised on the fly from Lance rows, taken per request with one `_rowaddr IN (...)` scan per leaf that holds a hit. Nothing caches them; `GET /_lance/stats` counts and times the takes under `fetch` (see [Cache statistics](#cache-statistics)). The body's `_source` element (`false`, an includes list, an includes / excludes object) is applied by OpenSearch's stock `FetchSourcePhase` over the synthesised source.
- `stored_fields`, `docvalue_fields`, `fields` and `"explain": true` are rendered by the same stock fetch sub phases an ordinary index runs, over the executor's fragment readers.
  ...
  - `docvalue_fields` reads the fragment readers' doc values (numeric, keyword, boolean, date with an optional `format`, patterns expanded against the mapping and bounded by `index.max_docvalue_fields_search`); a `lance_text` column has no doc values and is refused with the same `Fielddata is not supported` error an ordinary index answers.
  - `fields` reads the synthesised source.
  - `_explanation` explains a hit against the executor's query: a full text or `lance_knn` hit through the Lance Weight that served the page (its score from Lance's inverted index or nearest scan), a scalar filter the planner spelled as Lance SQL as `lance scan filter` with the constant score 1.0 (OpenSearch's shard search names its doc values query there instead).
- `from` + `size` pagination.
- `search_after` cursor pagination when the request carries a `sort` clause. A continuation of a sorted scalar page runs as the same ordered, limited Lance scan as its first page, with the cursor as a strict bound ANDed into the scan filter.
  - A cursor equal to a sort field's missing-value sentinel (a client paging past null rows) is served by the Lucene comparator, which knows the sentinel.
  - Cursor values are typed against the Lucene sort the way OpenSearch's shard search types them (`SearchAfterBuilder.buildFieldDoc`), so a sort whose first clause is `_score` takes a JSON double, and a cursor of the wrong length answers the same 400 an ordinary index answers.
  - A cursor without a Lucene sort (`sort` absent, or a lone descending `_score`, which builds no sort) answers 400 `Sort must contain at least one field.`, as an ordinary index does.
...
- Hits with equal scores or equal sort values are ordered by their Lance row address (`fragment id`, then offset) ascending, which is the doc id order of one Lucene reader over the whole table, so the order does not depend on how many nodes served the fragments; `_doc` sort means the same order.
  - Each executor ships the row address of every hit to the coordinator over the transport layer only; it is not part of the response.
  - Which tied rows a bare full-text page (no `sort`) contains at its cut is decided by Lance's bounded scan; see [limitations.md](limitations.md).
...
- `min_score` and `terminate_after` are applied by the same collectors OpenSearch's query phase composes around its query (`MinimumScoreCollector` outermost, the early terminating counter inside it), around the executor's hits, count and aggregation collections. A request with either knob collects on one slice and is never planned as a pushed page or a pushed aggregate.
  - A document below `min_score` is neither returned, counted nor aggregated; `hits.total` is then what the collectors counted rather than a Lance metadata count or the full text scan's hit count, for every query shape (`match_all`, a scalar filter, a pushed full text or `lance_knn` query).
  - `terminate_after` stops each executor after that many documents. The response says `terminated_early: true` when any executor stopped (and `false` when none did; the key is absent without the knob), and the aggregators see the documents collected before the bound.
  - Under `terminate_after`, `hits.total` is the collected count for a page and, for `size: 0`, the count Lucene's `TotalHitCountCollector` answers, which for `match_all` is the full document count next to `terminated_early: true`, exactly as on an ordinary index. With several executors the bound applies per executor, as it applies per shard on a multi shard index.
- `rescore` runs on every executor over its first pass the way `RescoreProcessor` runs it per shard: the first pass collects the largest `window_size` of the rescorers (at least `from + size`) through the Lucene collector, each rescorer re scores the window with its `rescore_query` under `query_weight`, `rescore_query_weight` and `score_mode`, the page is cut to `from + size` and the coordinator merges the executors' pages by the new scores. `hits.total` is the first pass count. A request with `rescore` is never planned as a pushed page.
  - With several data nodes each executor rescores its own window, as each shard does on a multi shard index; a `window_size` at least the number of matching rows makes the page independent of the node count.
  - A rescore query that is a Lance full text or `lance_knn` query runs one Lance scan of the table per rescorer on each executor (Lance scores the whole match set, not the window's rows), so rescoring with a Lance query costs a second scan next to the first pass, and `"explain": true` next to such a rescore runs one more scan per explained hit.
  - Refused as on an ordinary index, with OpenSearch's messages: `rescore` with a `sort` that builds a Lucene sort (`Cannot use [sort] option in conjunction with [rescore].`; a lone descending `_score` builds none), a `window_size` above `index.max_rescore_window`, and `rescore` in a scroll context (refused by the search request itself).
- `collapse` runs on every executor through the same `CollapsingTopDocsCollector` OpenSearch's query phase builds from the field's mapping (`keyword` or a number with doc values, one value per row), one collector per collection slice merged with `CollapseTopFieldDocs.merge`, with `from + size` groups per executor under the request's `sort` (score order without one). The coordinator merges the executors' pages and keeps the first hit of every value, so one hit per value survives across the nodes, then applies `from` and `size`.
  - `hits.total` counts every matching row, not the groups. The collapse value of every hit is rendered as the field's doc value under `fields`, as on an ordinary index.
  - `inner_hits` are expanded the way `ExpandSearchPhase` expands them: one multi search with, per collapsed hit and per block, the request's query wrapped in a `bool` that pins the hit's collapse value, under the block's `size`, `sort`, `_source`, `docvalue_fields`, `fields`, `stored_fields`, `explain`, `track_scores`, `version` and `seq_no_primary_term`. Each group search is an ordinary fragment path search, and `max_concurrent_group_searches` bounds how many run at once.
  - `search_after` with `collapse` needs the sort to be the collapse field alone. A request with `collapse` is never planned as a pushed page and its Lance scan is not clipped to the page.
  - Refused with OpenSearch's messages, as 400: a field that is not mapped, not a keyword or number, or without doc values; `collapse` with `rescore`; `collapse` with `search_after` under another sort; `collapse` in a scroll context.
  - `max_score` under `collapse` with a field sort and `track_scores` is not reported (an ordinary index reports the largest score of every match there).
- `track_total_hits` follows the OpenSearch contract: omitted counts up to 10,000 and reports `{"value": 10000, "relation": "gte"}` beyond that, an integer sets that bound, `true` counts exactly, `false` omits `hits.total`. The coordinator ships the bound to every executor and composes the relation from the per-node counts; whenever the relation is `gte` the value is the bound itself, as on an ordinary index, also when the per-node counts of several nodes add up to less than the bound because each node counted its own fragments' share of a scan stopped at the bound.
  - `match_all` is counted from Lance metadata and is always exact on the executor.
  - A scalar filter (`term`, `range`, `bool` of those) is counted by a Lance scan over the executor's fragments that returns no columns: under a bound the scan stops at `bound + 1` rows and the executor reports a lower bound, and with `track_total_hits: true` Lance counts the matches natively (`count(*)` on its side, one number back), so neither reads the matching rows into the JVM.
  - A Lance FTS query (`match`, `lance_*`, or a `bool` collapsed into a prefiltered FTS scan) is counted from the same scan that served the hits or aggregations when that scan saw every match, and otherwise by a count-only scan stopped at `bound + 1` rows, so a query with many hits costs the same as one with few until `track_total_hits: true` asks for the exact number.
...
- `"profile": true` renders `profile.lance.nodes.<node id>` with what each executor spent on the request. The fragment path runs no shard, so the stock `profile.shards` is not rendered. An answer served from the result cache ran on no executor and reports `profile.lance.cached: true` instead.
  ...
  - `fetch.take_count`, `fetch.take_rows` and `fetch.take_millis` count the `_rowaddr IN (...)` take scans the request issued on that node in either phase: the stored fields of a page are taken in the fetch phase, the sort column of a sparse full text or vector hit set in the query phase. A `size: 0` request reports zero takes. The same takes are counted node wide under `fetch` in `GET /_lance/stats`.

### Timeout and cancellation

- The request's `timeout` (else the cluster's `search.default_search_timeout`, else none; the plugin adds no default of its own) is the transport timeout of every per-node fragment request. A node that has not answered by then is logged once at `WARN` on the coordinating node (node id, index, fragment count, timeout) and its executor task is cancelled.
  - With `allow_partial_search_results` true (the request parameter, else the cluster's `search.default_allow_partial_results`, default true) the response is built from the nodes that answered: `timed_out: true`, `hits` from those nodes, `hits.total` as the sum of their counts with relation `gte`, and aggregations as the reduce of their partials. `_shards` keeps reporting one logical unit, so a node that did not answer is visible through `timed_out` and the log line, not through `_shards.failed`.
  - With `allow_partial_search_results: false` the request fails with HTTP 504 (`timeout_exception`, the transport timeout as the cause) once every node has answered or timed out.
- The per-node executors run as child tasks of the coordinator task (`internal:lance/coordinator_search`), which is itself a child of the search task (`indices:data/read/search`). Cancelling the search task or the coordinator task through `POST /_tasks/_cancel`, or a client that closes its connection before the answer, cancels every executor task (`internal:lance/fragment_query`) and ends the request with `task_cancelled_exception`.
  - Cancelling one executor task alone (`_tasks/_cancel?nodes=...&actions=internal:lance/fragment_query`) is treated like that node timing out: partial results with `timed_out: true`, or a failure under `allow_partial_search_results: false`.
...
The attach body and the namespace register body accept an `overrides` clause with per-column mapping rules, persisted in the `index.lance.overrides` index setting and re-applied on every manifest version advance. [mapping-overrides.md](mapping-overrides.md#mapping-overrides) describes each kind, the validation table and the persistence rules.

Seven kinds are supported: `type: date` on an epoch millis integer column, `type: keyword` on a Utf8 column with an inverted index, `type: ip`, `type: wildcard`, `type: geo_point` on a two float struct or `FixedSizeList<Float64>[2]`, `type: text_analyzer` for the OpenSearch analyzer mode over a derived tokens column backfilled with `lance.attach.backfill_threads`, and keyword sub-fields (`fields`).
...
- All run through OpenSearch's standard aggregator machinery over Lance-backed doc values, except the shapes [aggregations.md](aggregations.md) lists, which the scan computes. The coordinator reduces the per node results with the stock reduce.
  ...
  - A `filter` / `filters` bucket over a Lance full text or `lance_knn` clause runs one Lance scan per bucket on each executor for its `Weight`, as it does over the shard engine's whole table reader.
- A column an aggregation reads for the whole fragment (numeric, boolean, date, float, keyword) is loaded by each executor in up to `lance.fragment_path.parallelism` contiguous fragment groups scanned side by side on the `index_searcher` pool (dynamic, 1 to 32, default half the CPUs the JVM sees, at least 1; `1` is one scan per column).
  - The scan's Lance side decodes on Lance's own threads; the Java side that reads the batches into the column arrays is one thread per scan, which on a node with many fragments was the whole request's core budget.
  - Each fragment's arrays are written by exactly one group, so nothing is merged; when the pool has no free thread the request's own thread scans the remaining groups.
- Each executor collects its fragment leaves in up to `lance.fragment_path.slices` slices (dynamic, 1 to 32, same default as `lance.fragment_path.parallelism`), the way concurrent segment search slices a shard's segments. This applies to the top hits collectors and to every aggregation that runs through the aggregators; the pushdown shapes are not affected.
  - The leaves are bundled into slices by row count, every slice collects on its own `index_searcher` pool thread with its own collector (for aggregations, its own aggregator tree), and the slice results are reduced on the executor before the answer goes to the coordinator.
  - `1` collects every leaf on the request's own thread in fragment order and reduces nothing; a pool with no free thread leaves the remaining slices to the request's thread as well.
  - Under several slices a `terms` aggregator keeps every bucket of its slice and the executor's reduce applies `shard_size` and `shard_min_doc_count` once over the merged buckets, as a shard does with concurrent segment search on.
  - The tdigest `percentiles` and `cardinality` sketches are built per slice and merged, so their values can move with the slice count within the algorithm's error (see [limitations.md](limitations.md)).

### Aggregation pushdown

A `size: 0` request over `match_all` or a scalar filter whose tree is metrics only (tdigest `percentiles` included), a chain of up to three `terms` / `histogram` / `date_histogram` / `range` / `date_range` / `missing` / `filter` / `filters` levels with metric children, or one `composite` over `terms` / `date_histogram` sources runs as a Substrait `AggregateRel` inside the Lance scan (`LanceSubstraitProducer`, `ScanOptions.substraitAggregate`); the planner chooses it against the aggregators by cost.

`lance.aggregation.pushdown`, `lance.aggregation.pushdown_parallelism`, `lance.aggregation.pushdown_max_groups`, `lance.aggregation.percentiles_bins` and `lance.aggregation.pushdown_topk_slack` shape it. [aggregations.md](aggregations.md) has the shape list, the filter encodings, the `terms` / `percentiles` / `cardinality` behaviour and the settings table.
...
- The response is `{"index": ..., "route": ..., "logical": "...", "physical": "...", "fragment_plan": {...}, "unplanned": "...", "refinements_possible": [...], "traits": {...}}`. [query-plan.md](query-plan.md) is the field by field reference with an example.
  ...
  - `logical` and `physical` are the translator's tree and the planner's choice, one operator per line, every physical operator carrying the `accuracy` and `tie_stability` it declares and the `cost` the planner charged it, the root the `total_cost`.
  - `fragment_plan` is the `FragmentPlan` the coordinator ships to the data nodes.
  - `unplanned` names the request element that kept the envelope or the query on Lucene, or the `plan_failed` message when no plan meets the body's trait demand.
  - `refinements_possible` lists the node local downgrades a data node could still apply, in reason order; `traits` says what the body demanded, what the plan root declares and whether the enforcer fired.
- The plan is a costed choice, not a routing table: the pushdown rules fold what the Lance scan can compute into the scan, converter rules produce the Lucene alternative (`LuceneAggregateExec`, `HeapTopKExec`) for the same tree, and the Volcano planner keeps the cheapest candidate that declares the traits the request demands. See [query-plan.md](query-plan.md#cost) for the model and [architecture.md](architecture.md#planner-design) for the design.
...
- The per node plan and the explain response are wire formats internal to the plugin, as is every other message the plugin sends between nodes (the fragment request and response, the per node stats and build messages, the sync, poll, namespace update and attach messages). Each opens with a version marker and appends the fields of every later version as a block, so a node of the previous plugin version reads what it knows and steps over the rest, and a rolling upgrade between the current and the previous plugin version keeps the cluster answering.
  - A block whose absence would change the answer is flagged critical and a node of the previous version refuses it by name (`FragmentPlan wire version [3] adds fields in version [3] that this node's [2] cannot ignore: upgrade this node before sending it this message`).
  - [query-plan.md](query-plan.md#wire-format) lists the marked messages and [design/wire-format-compat.md](design/wire-format-compat.md) the rules and version history.
- Two request elements demand a trait of the plan root: an explicit `track_total_hits` demands an exact count (`Accuracy.EXACT`), a `search_after` cursor over a planned page demands a reproducible tie order (`TieStability.STABLE_KEY`). See [query-plan.md](query-plan.md#traits).
  - A body no plan meets (a `track_total_hits` bound next to a `cardinality` or `percentiles` aggregation, a cursor over a page sorted by `_score` alone) is refused by `_search` with a 400 naming the trait; explain answers the same body with the cheapest plan the demand refused, the message under `unplanned` and the refusal under `traits.enforcer`.
...
- The take is used while the hit set covers at most 0.25 percent of a fragment's rows (`LanceFragmentLeafReader.SPARSE_RATIO`); above that, or when no Lance clause drives the search, the fragment falls back to the full column load.
  - The threshold is the ratio of the two per-row costs: a take costs about 8 µs per row and the sequential column scan about 20 ns per row, so above one row in four hundred the scan is cheaper.
  - A column the node's off-heap column store already holds for the fragment (see [Snapshot and column cache](#snapshot-and-column-cache)) is read from the store whatever the size of the hit set, for numeric, boolean and keyword columns alike, because reading a held column costs no scan at all; a hit set below the ratio never starts a store load on its own.
  - Numeric columns also fall back per fragment the moment a doc outside the hit set is requested (a `should` with another clause, a `should` beside a `filter`), so results do not depend on the query shape. Keyword columns build their ordinal dictionary from the hit rows only when the Weight has shown that nothing else is collected on that fragment.
  - Under a reader wrapper installed by another plugin (the security plugin's document and field level security reader) sparse keyword dictionaries are disabled and keyword columns load fully, because such a wrapper can hide rows the Lance scan returned and a dictionary built from every hit row would expose their terms through its value count; numeric and boolean columns keep the per-hit take, since they are read only for the documents the wrapper lets through.
- When the top-level query is the bare Lance clause (alone, or with a pushed-down `filter` / `must_not`; a `post_filter` may be present) and the request carries aggregations or a sorted page, the fragment executor runs the clause's scan and delivers the per-fragment hit set to the readers before it builds the aggregators and sort comparators. The same scan then serves the hits phase and the aggregations.
  - A keyword `terms` aggregation (global ordinals are built when the aggregator is created, also with `size: 0`) and a keyword sort (the comparator looks the current bottom term up when the leaf comparator is created) therefore read the dictionary of the hit rows on every fragment.
  - A Lance clause nested in a `bool` with other Lucene clauses, or wrapped in `constant_score`, gets no early hint and follows the Weight-driven rules above.
...
- The coordinator cuts each data node's fragments into contiguous groups whose physical rows fit the bound and sends one fragment request per group (the coordinator log line `lance.dispatch: fan-out index [...] to node [...] with fragments [...]` then ends in `(group i of n, rows)`). Tables under the bound keep one request per node.
  - The merge reduces a group's response like a node's: hits are sorted by the request's sort with the row address as tie break, aggregations go through the stock reduce, exact totals add up, and a bounded `track_total_hits` reads `gte` at the bound once the groups' shares pass it.
...
Test settings, all node scope and dynamic; do not change them on a real node:
...
- The shard engine's whole-table reader (behind `GET /_doc/{id}`, `_stats` and a `_search` over a mixed target) is a view over the same snapshot. Opening the shard leases the snapshot of the version it serves; a refresh that follows the table to a new version leases that version's snapshot and releases the previous one when its last searcher closes.
  - A node therefore holds one open dataset, one fragment list and one set of store columns per table version, whichever path touched it first, and the first `_search` after a shard opened builds nothing.
  - The engine never retires a snapshot; the freshness check on the shard's node does, after it has refreshed the shard.
- The coordinator opens the table once per request to enumerate its fragments and ships the manifest version it observed (the pinned or tag version, or the latest one) with every per-node request, so every executor of one request reads the same version and keys its snapshot on it without any Lance call. An index that follows the latest manifest therefore sees an append on the next `_search`; a new version gets its own snapshot.
  - An executor that receives no version (none is sent by the coordinator in this release; the fallback exists for older callers) resolves the latest version itself, from the newest cached dataset's `Dataset.latestVersion()` or by opening the table.
- Numeric, boolean, date and float columns a request reads for the whole fragment (sort or aggregation without a usable full-text or vector hit set, `_source` rendering aside) are loaded once per `(snapshot, column, fragment)` into an off-heap column store and kept there: Arrow `BigIntVector` / `BitVector` buffers allocated from a child of the plugin's Arrow allocator, holding the same normalised `long` values the heap path held (sortable long for floats, epoch millis for dates). Doc values read the buffers directly.
  - One Lance scan per column covers every fragment the request needs and does not hold yet; the query's filter is not applied to that scan, so the slice serves every later query.
- Keyword and keyword array columns (including the base column of a `keyword` multi-field) are kept in the same store: per `(snapshot, column, fragment)` a sorted, duplicate-free term dictionary (`VarCharVector`) plus one ordinal per row (`IntVector`; keyword arrays add a row offset vector over the flattened ordinals). The load scans the column once, interns the values in heap for the duration of the load, checks the resulting size against the budget, and writes the vectors; later requests read the ordinals and terms in place and rebuild nothing.
  - When the dictionary does not fit (the budget is exhausted by columns running requests read, or `lance.cache.column_share` is 0), the request keeps the dictionary and ordinals that scan produced as its own heap columns, so the column is scanned once either way; a zero budget skips the store before the scan.
  - `lookupOrd` copies the term into a scratch owned by the doc values instance.
  - Two keyword loads stay on the request scoped heap path: the dictionary of a small full-text or vector hit set ([Doc values for full-text and vector hits](#doc-values-for-full-text-and-vector-hits)), and a request whose top-level filter was pushed into the column scan (its ordinals cover the matching rows only, so the result cannot be shared).
- Order of the doc value sources on a fragment: the rows taken for a small full-text or vector hit set, then the off-heap store, then a store load, then a request scoped heap load when the store has no room. Eviction is least recently used at `(column, fragment)` granularity and never touches a column a running request reads; a request that finds no room loads into heap for itself (`DEBUG` log `column cache budget exhausted`).
- Snapshots are closed when the freshness check sees the table (or its tag) move to another version, when the index is deleted, when `lance.cache.enabled` is set to `false`, or when they are the least recently used ones above `lance.cache.max_snapshots`; a snapshot a request or a shard reader still holds closes when that holder ends.
  - Closing a snapshot releases its columns, unless a newer snapshot of the same `(index uuid, version)` has taken its place (a shard reader can outlive a `lance.cache.enabled` off and on), in which case the columns stay with the replacement.

| setting | scope | default | dynamic | effect |
|---|---|---|---|---|
| `lance.cache.enabled` | node | `true` | yes | `false` retires every snapshot; from then on each request and each shard reader opens the table, runs the schema pass and loads its columns into heap for itself, and concurrent requests against the same index share none of that |
| `lance.cache.max_snapshots` | node | 64 | no | only snapshots nobody holds are evicted, so shard readers never count against it |
| `lance.cache.column_share` | node | 0.4 | no | the fraction of `lance.native_memory.limit` the column store may use; see [Native memory bounds](#native-memory-bounds) |
...
- `column_store`: `bytes` (off-heap bytes the store holds), `limit_bytes` (its budget), `entries` (`(snapshot, column, fragment)` slices held), `hits` (column requests answered without a scan), `loads` (scans run to fill slices), `evictions`, `budget_misses` (requests that loaded into heap because the budget could not be met).
  - `heap_fallback_bytes`: heap bytes those loads currently have charged to the request circuit breaker across the node's open readers; zero once every reader that charged has closed.
  - `heap_fallback_rejections`: heap loads the request breaker refused, each of which ended a request with HTTP 429.
...
- `request_cache`: the coordinator result cache of the node (see [Result cache](#result-cache)): `enabled` (current `lance.request_cache.enabled`), `size_bytes` (what the entries weigh), `limit_bytes` (`lance.request_cache.size` in bytes), `entries`, and the counters since the node started. A node whose plugin version predates the cache reports it disabled with zero counters.
  - `hits`: requests answered from an entry. `misses`: eligible requests the cache did not hold, each of which stored its answer unless it timed out or was above the entry bound.
  - `evictions`: entries dropped for room or because `lance.request_cache.expire` passed. `invalidations`: entries dropped because their index was deleted or `POST /{index}/_cache/clear` named it.
  - `skipped`: requests the cache did not take: hits requested, several target indexes, `request_cache=false`, a reader wrapper on the index, a timed out answer, an answer above the entry bound, an answer the store threw on.
- `native_memory`: `estimated_bytes` (what the `lance_native` breaker currently accounts for, as of the last sampler tick), `session_bytes` (`Session.sizeBytes()` of the shared Lance session, read live), `column_store_bytes` (the same value as `column_store.bytes`), `index_cache_capacity`, `index_cache_shards` and `index_cache_shard_share` (the index cache handed to Lance at startup, the shard count Lance derives from it and the heaviest entry one shard admits).
  - The breaker estimate is the sum of `session_bytes` and `column_store_bytes` as sampled up to one `lance.native_memory.circuit_breaker.poll_interval` ago.
- `fts`: `subset_probe_limit` (current `lance.fts.subset_probe_limit`).
- `admission`: the decisions of the [admission gate](admission.md).
  - `enabled` and `headroom_bytes`: the admission settings in force.
  - `available_bytes`: the node's available physical memory when the stats were collected, `MemAvailable` on Linux and the free physical memory elsewhere, before the headroom is subtracted.
  ...
  - `rejections`: an object with one counter per kind (`fts`, `scalar_index`, `vector_index`, `filter_scan`, `aggregate_scan`, `column_load`) of the scans the node's gate refused since it started, every counter present even at zero. A warm up probe the gate did not admit counts under `fts`.
...
- `fetch`: the `_rowaddr IN (...)` take scans the node's fragment executors issued since it started. No cache serves these scans. `take_millis_total` against the `took` of the requests a node served over the same interval is the share of the request time the takes account for; a `size: 0` aggregation issues none.
  ...
  - `take_millis_total`: wall time from each scan's creation to its close, decoding included.
...
- The plugin keeps its own result cache on every node that coordinates requests: the reduced answer of a `size: 0` request against one Lance backed index (its aggregations, `hits.total`, `terminated_early`), served again while the table stays at the version it was computed from. The fragment path does not reach OpenSearch's shard request cache (`indices.requests.cache.*`, whose counters stay 0 for a Lance backed index).
  ...
  - A hit sends no per node request. Its cost is the table's current version for the key: an index that follows the table reads the manifest (about 1 ms on a local disk, 10 to 15 ms over S3), an index pinned to a version has it in its settings and is looked up before the table is opened, and an index following a tag resolves the tag (one manifest read) and is then looked up the same way.
- The key is `(index uuid, manifest version, data node ids, canonical body)`.
  ...
  - The canonical body is the search body with every object's keys sorted and the whitespace removed, so two bodies that spell the same request in another key order share an entry; array order is kept (the order of `sort` and `should` clauses is part of the request).
  - Everything in the body is in the key, `timeout`, `terminate_after`, `profile`, `explain` and `track_total_hits` included; the request level options `preference`, `request_cache` and `allow_partial_search_results` are not.
  - The key holds the request and the table version, not the node settings a request runs under: a change of `lance.fragment_path.slices`, `lance.aggregation.pushdown` or another execution setting does not drop the entries, and a body answered before the change keeps its answer until the table moves on or the cache is cleared (`POST /{index}/_cache/clear?request=true` after such a change, when the difference matters, as a `percentiles` sketch can move with the slice count).
- A request is cached when its body asks for no hits (`size: 0`, and no `from`; `_count` qualifies), its target resolves to one Lance backed index, it does not carry `request_cache=false`, and the index has no reader wrapper installed.
  - A target of several indexes, Lance backed or not, is not cached.
  - The security plugin's document and field level security shape the answer per user, so an index with a reader wrapper is never cached; the check builds a temporary index service on a coordinating node that holds no shard of the index, once per index.
  - An answer that timed out (`timed_out: true`, a partial answer under `allow_partial_search_results`) is not stored, nor is one whose reduced aggregations serialise above `lance.request_cache.max_entry_size`.
  - `request_cache=false` on the query string opts one request out, as it does on the stock search; `request_cache=true` is the default and needs no spelling. `_count` is a `size: 0` search with `track_total_hits: true` and shares that body's entry; it takes no `request_cache` parameter, so it is opted out only by the setting.
...
| setting | scope | default | dynamic | effect |
|---|---|---|---|---|
| `lance.request_cache.enabled` | node | `true` | yes | `false` drops every entry and caches nothing |
...
- When a Lance-backed index appears in the cluster state (attach, namespace poll, node restart), every data node reads the table's indexes into its own Session cache in the background (`LanceIndexWarmer`, on the single threaded `lance_warm_up` pool, one table at a time, one index at a time). The attach response does not wait for it and a request that arrives while it runs loads what it needs on its own.
  - Why: Lance loads an index the first time a scan uses it (BTree page lookup then one object store request per matching page; full-text token dictionaries; IVF centroids and the probed partitions' codes) and keeps the loaded parts in the Session index cache, so the first request that uses an index on a table read from S3 pays for the load on its own thread, in latency bound page reads.
- `lance.attach.warm_indexes` (node setting, dynamic, default `metadata`) selects how much is read.
  - `none` reads nothing.
  - `metadata` opens every index (BTree page lookup, bitmap keys, full-text token dictionaries, IVF centroids and one partition) through a scan that projects no data column and takes at most one row (`col IS NULL`, a full-text match on a token that exists nowhere, `nearest` with `k = 1` and one probe).
...
| Arrow type | OpenSearch mapping |
|---|---|
| `int8` / `int16` / `int32` / `int64` (signed) | `byte` / `short` / `integer` / `long` |
| `uint64` declared as the primary key | `unsigned_long` |
...
- Signed integers: unsigned integer variants are noted in the attach response and left unmapped, except for UInt64 declared as the primary key.
- `uint64` primary key: values are held as raw 64-bit patterns; `Long.toUnsignedString` decodes the `_id`, and `BigInteger` handles the `GET /_doc/{id}` parse. Non-PK UInt64 columns are still not surfaced.
- `float32` / `float64`: values are folded into the shared `long[]` doc value storage via `NumericUtils.floatToSortableInt` / `doubleToSortableLong` and decoded on the way out so range, sort, metric aggregation, and `_source` all round trip. `float16` stays unmapped.
- `date` / `timestamp`: normalised to epoch millis in the reader.
- `utf8` with a Lance FTS index: enables `match`, `lance_match`, `lance_match_phrase`, `lance_multi_match`.
- `utf8` without a Lance FTS index: SortedSetDocValues, so `term` / `terms` / `terms` aggregation work.
- `struct`: children map recursively under `properties` (int widths, float/double, boolean, date/timestamp, `utf8` → `keyword`, `list<utf8>` → `keyword`); term / terms / range / exists, sort, and aggregations resolve on the `parent.child` path, and `_source` renders the nested JSON object. FTS and knn are not derived on struct children; a child type outside this list is noted in the attach response and skipped while the parent object is still emitted.
- `list<struct>`: each list element becomes a hidden child document before its row's parent document, so a `nested` query can match several attributes of the same element. See the Nested subsection below.
- `fixed_size_list<float32>`: dimension carried through the mapping; `lance_knn` validates it.
- `binary` / `large_binary`: Base64 in `_source`, no doc values.

Multi-fields (`fields.raw: keyword` on a Utf8 base column), a `date` override on an epoch-millis integer column, a `keyword` override on an inverted-index Utf8 column, an `ip` override on a Utf8 column of address strings, and a `wildcard` override served by the keyword path are supported through the `overrides` attach clause ([Mapping overrides](#mapping-overrides)).
...
- **`lance.native_memory.limit`** — Absolute (`10gb`) or a percentage of host memory left after the JVM heap (`40%`, default). Static, node scope; rolling restart to change. The resolved split, shard count and share are logged at startup.
  - `lance.cache.column_share` (default 0.4) of it goes to the off-heap column store; the rest is split 6:1 between the index cache and metadata cache.
  - The index cache receives the capacity within its 6/7 budget whose share per Lance cache shard is largest (Lance refuses an entry heavier than one shard's share, and the shard count grows with the capacity); the difference stays unused.
...
- **Admission gate** — Before each gated Lance scan (full text, scalar index load, filter scan, nearest, pushed aggregate, heap column load) the executor estimates the native memory Lance will allocate and refuses the request with 429 `circuit_breaking_exception` under the label `lance_admission` when the node's available physical memory minus `lance.admission.headroom` (dynamic, default `8gb`), plus what earlier admitted scans of the same identity retained, cannot hold it. [admission.md](admission.md) has the estimators per kind, the retained memory credit and the 429 message.
  - `lance.admission.enabled` (dynamic, default `true`) turns it off; `lance.admission.bounded_shapes_gated` (dynamic, default `true`) gates bounded pages.
- **`request` circuit breaker (heap side)** — The hits of a full-text or knn query are buffered on heap for the life of the request (8 bytes per matching row of the node's fragments, plus 8 bytes per row of each fragment's sorted view) and every buffer is reserved with OpenSearch's standard `request` breaker under the label `lance_fts_hits` before it is allocated, then returned when the request ends.
  - A hit set the breaker refuses (an unbounded shape such as a sort by a field or an aggregation over a query that matches a large share of a large table) answers 429 `circuit_breaking_exception` instead of exhausting the heap; `indices.breaker.request.limit` bounds it.
  - The Lance scan behind a full-text or knn query returns `_rowaddr` and `_score` (`_distance` for knn) only, so its native buffers hold about 12 bytes per matching row whatever columns the table has.

Cache lifecycle: deleting a Lance-backed OpenSearch index (via `DELETE /<index>`) reinstalls the shared `Session` on the node the DELETE lands on. This drops every cached index / metadata page held by the Session so that a later re-attach against the same table URI cannot pick up stale `_indices/<uuid>/` pointers left behind by the deleted table.

- Every other Lance-backed index on that node pays a cold cache cost on its next query; the trade-off is unavoidable today because Lance's Java SDK does not expose a per-path invalidation hook.
- `CLOSED`, `NO_LONGER_ASSIGNED`, and other non-delete removal reasons keep the cache intact because they leave the underlying files in place.
